	...
	$.fn.touchslide = function(options) {
		...
		this
				.each(function() {
					...
					if (imgcount > 1) {
						$(
								(slideImgWrapper.html().split("/a>")[0]
										+ "/a>"
										+ slideImgWrapper.html().split("/a>")[1] + "/a>"))
								.insertAfter(slideAs.last());
						if (navshow) {
							$("<div class='navimgs'></div>").insertAfter(
									slideImgWrapper);
							...
							for ( var i = 0; i < imgcountrealy; i++) {
								circlewrapper
										.append("<canvas class='circle' width='15' height='15'></canvas>");
							}
					...
					function navclick(navnum) {
						circlewrapper.children('canvas:eq(' + navnum + ')')
								.click(function(e) {
					...
					function scideimgskip(imgnum) {
						...
						slideImgWrapper.stop(true).animate({
							left : -turnleftwidth
					...
					function canvacircle(canvanum) {
						...
						for ( var i = 0; i < circles.length; i++) {
							if (canvassuport) {
								...
								cxt.arc(circler * 0.5, circler * 0.5,
										circler * 0.5, 0, Math.PI * 2, true);
								cxt.closePath();
								cxt.fill();
							} else {
								circles.css("background", "#ddd");
								circlewrapper.children(
										'canvas:eq(' + canvanum + ')').css(
										"background", "#0182D7");
							}
					...
					function turnright() {
						...
						if (turnleftwidth == 0) {
							slideImgWrapper.css({
								'left' : -slideWrapperWidth * (imgcount - 1)
							});
					...
					function mouseDrag($element) {
						...
						for ( var i = 0; i < slideAs.length; i++) {
							if (typeof (slideImgWrapper.children('a:eq(' + i
									+ ')').attr('href')) !== 'undefined') {
								ahrefs.push(slideImgWrapper.children(
										'a:eq(' + i + ')').attr('href'));
							}
						}
						eventEle
								.mouseover(function() {
									for ( var i = 0; i < slideAs.length; i++) {
										if (typeof (slideImgWrapper
												.children('a:eq(' + i + ')')
												.attr('href')) !== 'undefined') {
											slideImgWrapper.children(
													'a:eq(' + i + ')').attr(
													'href', ahrefs[i]);
										}
									}
